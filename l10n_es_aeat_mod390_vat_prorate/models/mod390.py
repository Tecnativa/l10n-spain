...
class L10nEsAeatMod390Report(models.Model):
    ...
    def check_prorate_ids_total_deduction_valid(self):
        for item in self:
            if item.prorate_ids:
                amount = sum(x['amount'] for x in item.prorate_ids)
                key = 'amount_allow_deduction'
                amount_deduction = sum(x[key] for x in item.prorate_ids)
    ...
    def button_confirm(self):
        ...
        return super(L10nEsAeatMod390Report, self).button_confirm()
            ...
            if item.vat_prorate_type != 'none':
                ...
                item.prorate_ids = [
                    (0, 0, {
                        ...
                        'cnae': 'CNAE',
                        'amount': amount,
                        'amount_allow_deduction': amount,
                        'type': item.vat_prorate_type,
    ...
    def _prepare_tax_line_vals(self, map_line):
        ...
        if (self.vat_prorate_type == 'general' and
                map_line.field_number in PRORATE_TAX_LINE_MAPPING):
            reduction = (res['amount'] / 100) * self.vat_prorate_percent
            res['amount'] = res['amount'] - reduction
        return res
